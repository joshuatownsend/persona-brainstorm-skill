...
Ask for an **enrichment pass** and every item expands into the situation that produced it, the pressure behind it, and either how it's answered today or what would have to exist — written to a sibling of the core document, with a topic axis that crosses the persona one. A shareable page is offered on top of that, once; decline it and the markdown is still written, which is the deliverable and the source of truth in either case.
