...
Ask for an **enrichment pass** and every item expands into the situation that produced it, the pressure behind it, and either how it's answered today or what would have to exist — written to a sibling of the core document, with a topic axis that crosses the persona one. A shareable page is then offered on top of that; the markdown is the deliverable either way, and declining changes nothing else about the pass.
