...
**Frequencies:** ペルソナからの推定であり、実測ではない. No interviews, tickets, logs or
...
|---|---|---|---|---|---|---|
| 1 | "I've got maybe twenty minutes before I lose the thread on this feature — give me the version of this that fits in twenty minutes and tell me what I'm giving up by not doing the full thing." | Whether to run discovery now at all, or defer it and ship on instinct | 何もしない | weekly | | ◐ |
| 2 | "I already know my users. Skip the ones I'd have named myself and only show me the ones I wouldn't have." | Whether the run produced anything worth reading, in the first ten seconds | 何もしない | weekly | | ◐ |
| 3 | "Which of these needs is actually just me, projecting?" | Whether to build a thing one person wants and call it a user need | 何もしない | weekly | | ○ |
| 4 | "I ran this three months ago before I rewrote the onboarding — what would be different if I ran it today?" | Whether the old discovery doc still governs the roadmap or should be thrown out | 何もしない | monthly | ⚡ | ○ |
| 5 | "Nine of these ten needs I'm never going to serve. Which one is the one I'd regret ignoring in a year?" | What single thing goes on next week's list | 何もしない | weekly | | ◐ |
| 6 | "Tell me the need that, if it's real, means the thing I've been building for six weeks is the wrong thing." | Whether to kill in-flight work before sinking more time | 何もしない | monthly | ⚡ | ○ |
| 7 | "I don't have users yet. Do this anyway and be honest about how much of it you made up." | Whether to trust the output enough to pick a launch wedge | 何もしない | monthly | | ◐ |
| 8 | "Don't give me personas. Give me the four sentences I should put on the landing page." | What copy ships tomorrow | 何もしない | monthly | | ○ |
| 9 | "Which of these would show up as a support email, and which would show up as a silent uninstall?" | Where to spend limited instrumentation and where to spend limited care | 何もしない | monthly | | ○ |
| 10 | "I'm tired. Just tell me what to do next and hide the reasoning unless I ask." | Whether the session ends in an action or in another document to read later | 何もしない | weekly | | ◐ |
| 11 | "Run this on my competitor instead of me and tell me who they're serving that I'm not." | Whether there's an underserved flank worth aiming at | 何もしない | quarterly | | ○ |
| 12 | "Keep this to what a one-person team can actually serve — if a need requires a support org, say so up front instead of at the end." | Whether a need is even in scope for a solo shop before he emotionally commits | 何もしない | weekly | | ○ |
| 13 | "Last time you gave me generic startup advice dressed as user research. How do I stop that from happening again without reading every line?" | Whether to keep using the method at all | 何もしない | monthly | ⚡ | ◐ |

### P2 — The AI agent executing the method

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 14 | "What do I have to establish about this subject before I'm allowed to generate a single persona?" | Whether to proceed to generation or halt and gather | 何もしない | per-run | | ✅ |
| 15 | "I have no expertise in this domain. What is the minimum I must read, and how do I know when I've read enough versus when I'm just reading?" | When to stop orienting and start producing | 何もしない | per-run | | ✅ |
| 16 | "Show me what an under-reached run looks like next to a good one, so I can tell which one I just produced." | Whether to accept the output or re-run with more reach | 何もしない | per-run | ⚡ | ◐ |
| 17 | "The user told me their users are X. Everything I can observe says Y. What do I do with that, and am I allowed to say it to their face?" | Whether to defer to the stated brief or contradict the person paying for the run | 何もしない | per-run | ⚡ | ◐ |
| 18 | "Which of these steps can I skip when the subject is small, and which ones are load-bearing no matter what?" | Whether to run the full method or a reduced one under time pressure | 何もしない | per-run | | ◐ |
| 19 | "I'm about to produce sixty confident-sounding items about a field I've never worked in. What is the specific failure mode I'm walking into right now?" | Whether to add a caveat, gather more, or change approach before writing | 何もしない | per-run | ⚡ | ◐ |
| 20 | "Make me fail the way I fail in production — give me a subject where I reliably produce plausible hollow output, and let me see myself do it." | Whether the method's guardrails actually catch the agent's real failure mode | 何もしない | per-release | ⚡ | ○ |
| 21 | "Which parts of what I just wrote came from the subject, and which came from my priors about products in general?" | Whether to keep an item or cut it as generic filler | 何もしない | per-run | ⚡ | ○ |
| 22 | "Am I inventing users, or recovering them? Tell me which mode I'm in for each persona." | Whether the output is a hypothesis set or a claim, and how to label it | 何もしない | per-run | | ◐ |
| 23 | "The instructions I was given conflict with each other — one says be exhaustive, one says be short. Which one wins, and who decided?" | Which constraint to violate when both can't hold | 何もしない | per-run | | ○ |
| 24 | "I've got a budget of thirteen items and eight real ones. Do I pad, or do I hand back eight and say so?" | Whether to fabricate to hit a number or break the contract honestly | 何もしない | per-run | ⚡ | ✅ |
| 25 | "The method changed between the last run and this one. Which parts of the old output are still valid and which have to be regenerated?" | Whether to diff or discard prior output after a method upgrade | 何もしない | per-release | ⚡ | ○ |
| 26 | "I want to override the persona set the method derived and substitute my own — what breaks downstream if I do?" | Whether a manual override is safe or silently invalidates later steps | 何もしない | per-run | | ◐ |
| 27 | "Before I hand this over: what would a domain expert reading this immediately know was wrong?" | Whether to ship the output or flag specific claims as unverified | 何もしない | per-run | ⚡ | ○ |
| 28 | "I can't talk to a single real user. What is the strongest thing I'm still entitled to claim, and where exactly does my entitlement stop?" | How firmly to word the conclusions, and what the consumer may act on | 何もしない | per-run | | ◐ |

### P3 — Staff engineer scoping a system that doesn't exist yet

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 29 | "Nothing is built. Who would use this thing in year three that nobody is thinking about in year one?" | Whether the architecture needs an extension point now or can be closed | 何もしない | quarterly | | ◐ |
| 30 | "Which of these needs, if real, changes the data model? Sort by that, not by how many people want it." | Which decisions are cheap now and catastrophic later | 何もしない | quarterly | ⚡ | ◐ |
| 31 | "Who already built this and what did their user set turn out to be after two years in production?" | Build versus buy versus adopt the existing thing | 何もしない | quarterly | ⚡ | ○ |
| 32 | "Give me the demand picture under the assumption we're wrong about the primary user — who's the second-most-likely primary?" | Whether the design survives being wrong about its main audience | 何もしない | quarterly | | ○ |
| 33 | "Which needs here are actually the same need wearing two different job titles?" | Whether to build two subsystems or one with a flag | 何もしない | quarterly | | ◐ |
| 34 | "I need the list of users who will be forced onto this by a migration, not the ones who choose it." | Whether the migration path is a feature or an afterthought | 何もしない | quarterly | ⚡ | ◐ |
| 35 | "Tell me which of these needs will be served by something else entirely within eighteen months so I don't build it." | Whether to scope a capability in or bet on the ecosystem covering it | 何もしない | annually | | ○ |

### P4 — PM defending a roadmap

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 36 | "The VP is going to ask where number three came from. Where did number three come from?" | Whether the ranking survives the roadmap review | 何もしない | per-release | | ✅ |
| 37 | "Show me the sentence I can say out loud that makes this ranking defensible without me claiming we did research we didn't do." | How to present it honestly without losing the room | 何もしない | per-release | | ◐ |
| 38 | "An engineer says persona four doesn't exist. Give me the counter-argument, or tell me they're right." | Whether to hold the line on a contested item or drop it | 何もしない | per-release | ⚡ | ✅ |
| 39 | "What changed since last quarter's version, and which of those changes were because the world changed versus because the method changed?" | Whether to explain a reprioritization as new information or as noise | 何もしない | quarterly | ⚡ | ◐ |
| 40 | "Which items here are the ones I should NOT put in front of a customer, and why?" | What goes in the customer-facing deck versus the internal one | 何もしない | per-release | | ○ |
| 41 | "Attach a confidence to each of these that I can put in a slide without lying." | Whether to commit a date against an item or hedge it | 何もしない | per-release | | ○ |
| 42 | "Sales is going to say 'the biggest account wants X.' Where does X sit in this, and is it actually a need or one loud person?" | Whether to take the escalation or push back with evidence | 何もしない | weekly | | ◐ |
| 43 | "If this ranking is wrong, what's the first thing we'd observe, and how long until we'd see it?" | What to instrument now so the bet is falsifiable before the next planning cycle | 何もしない | quarterly | ⚡ | ○ |

### P5 — Consultant working in a client's domain

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 44 | "Which claims in here will get me caught by someone who's worked in this industry for twenty years?" | Which lines to verify with the client before the readout, and which to cut | 何もしない | weekly | ⚡ | ○ |
| 45 | "Teach me this domain's vocabulary well enough that I don't say 'customer' when they say 'member.'" | Whether the deliverable reads as insider work or as an outsider's guess | 何もしない | onboarding | | ◐ |
| 46 | "Separate what I learned from the client's own materials from what I'm inferring, and label every line." | What can be presented as finding versus hypothesis in the final report | 何もしない | weekly | | ◐ |
| 47 | "The client's stated problem and the demand picture don't match. Give me the framing that lets me say that without losing the engagement." | Whether to challenge the brief or deliver against it | 何もしない | per-run | | ◐ |
| 48 | "Two weeks from now I hand this over and disappear. What do they need to be able to re-run it themselves?" | What goes in the handover package versus stays in the consultant's head | 何もしない | quarterly | | ✅ |
| 49 | "Show me the three questions to ask in the stakeholder interview that would most change this output." | How to spend a single hour of scarce access to real people | 何もしない | weekly | ⚡ | ○ |

### P6 — Platform team running it across many subjects

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 50 | "Same input, same output — I need to run this twice and get the same thing, or the diffs are meaningless." | Whether run-to-run changes can be treated as signal in CI | 何もしない | per-run | ⚡ | ○ |
| 51 | "Diff two runs of the same service and tell me which changes are substantive versus reworded." | Whether a re-run needs human review or auto-merges | 何もしない | per-release | ⚡ | ○ |
| 52 | "Across all forty services, which user need shows up everywhere? That's a platform capability, not forty features." | What the platform team builds centrally versus pushes to service owners | 何もしない | quarterly | | ○ |
| 53 | "Give me this as structured data with stable IDs so my dashboard doesn't break when the wording changes." | Whether the output can be a pipeline input or has to be read by a person | 何もしない | per-run | | ◐ |
| 54 | "Which of the forty runs should I not trust — flag the ones where the input was too thin to support the output." | Where to spend the human review budget | 何もしない | per-release | ⚡ | ◐ |
| 55 | "Let me pin the method version per service so a mid-quarter change doesn't invalidate thirty reports at once." | Whether to upgrade all at once or stagger the rollout | 何もしない | per-release | ⚡ | ○ |

### P7 — Representation auditor

| # | The ask, in their words | Why — the decision it feeds | Today | Freq | ⚡ | Cov |
|---|---|---|---|---|---|---|
| 56 | "Who is missing from this list, and don't tell me 'no one' — tell me the process by which you'd know." | Whether the persona set can be signed off or has to go back | 何もしない | per-run | ⚡ | ◐ |
| 57 | "Where are the people who tried this and left? They don't file tickets and they don't answer surveys." | Whether the roadmap is optimizing for survivors and calling it demand | 何もしない | per-run | ⚡ | ○ |
| 58 | "Show me the version of each persona who's using a screen reader, on a bad connection, in a language this wasn't written in." | Whether accessibility and localization are requirements or a later ticket | 何もしない | per-run | | ○ |
| 59 | "You generated these from text written mostly by and about a certain kind of user. Name that skew out loud." | Whether to weight the output down or commission real research to correct it | 何もしない | per-run | ⚡ | ○ |
| 60 | "Which of these personas did you include because they're real, and which because leaving them out would look bad?" | Whether the persona set is honest or performatively inclusive, which changes what gets funded | 何もしない | per-run | | ✅ |
